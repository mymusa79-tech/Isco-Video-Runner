...
class NativeShortStageContractTests(unittest.TestCase):
    ...
    def test_run168_provider_prompt_mirrors_current_stage_semantics_for_all_short_stages(self):
        for stage_kind in ("short_draft", "short_review", "short_repair"):
            with self.subTest(stage_kind=stage_kind):
                ...
                provider_prompt = short_contract._provider_contract_prompt("ORIGINAL moment 7-20 seconds", spec)
                ...
                self.assertIn("between 12 and 20 inclusive", provider_prompt)
                self.assertIn(self.TOPIC, provider_prompt)
                self.assertNotIn("moment 7-20 seconds", provider_prompt)
                self.assertIn("moment 12-20 seconds", provider_prompt)
