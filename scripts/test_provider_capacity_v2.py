...
class ProviderCapacityV2Tests(unittest.TestCase):
    ...
    def test_malformed_cached_response_is_rejected_and_refetched(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "cache.json"
            key = capacity._canonical_key(
                media_kind="video", query="road", orientation="landscape", per_page=12
            )
            path.write_text(json.dumps({
                "schema_version": capacity.CACHE_SCHEMA_VERSION,
                "entries": {key: {"fetched_at": 1000.0, "response": [{"id": 1}, "bad"]}},
            }), encoding="utf-8")
            cache = capacity.PersistentPixabaySearchCache(path)
            with patch.object(capacity.time, "time", return_value=1001.0):
                result = cache.get_or_fetch(
                    provider="pixabay", media_kind="video", query="road",
                    orientation="landscape", per_page=12, fetch=lambda: [{"id": 2}],
                )
            self.assertEqual(result, [{"id": 2}])
            self.assertEqual(cache.misses, 1)

    def test_malformed_provider_response_is_never_persisted(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "cache.json"
            cache = capacity.PersistentPixabaySearchCache(path)
            with self.assertRaisesRegex(RuntimeError, "invalid normalized results"):
                cache.get_or_fetch(
                    provider="pixabay", media_kind="video", query="road",
                    orientation="landscape", per_page=12, fetch=lambda: [{"id": 1}, "bad"],
                )
            self.assertFalse(path.exists())
    ...
    def test_prune_bounds_persistent_cache_to_64_entries(self) -> None:
        entries = {}
        now = 10000.0
        for index in range(80):
            entries[f"k-{index}"] = {"fetched_at": now - index, "response": [{"id": index}]}
        pruned = capacity._prune(entries, now=now)
        self.assertEqual(len(pruned), capacity.PIXABAY_CACHE_MAX_ENTRIES)
        self.assertIn("k-0", pruned)
        self.assertNotIn("k-79", pruned)
