...
class ShortsProductionBindingTests(unittest.TestCase):
    ...
    def test_finalize_requires_gold_and_all_existing_hard_gates(self):
        root = self._root()
        pre = {
        ...
        report = binding.finalize_short_quality(root, self._request(), pre)
        ...
        with self.assertRaisesRegex(RuntimeError, "hard gates"):
            binding.finalize_short_quality(root, self._request(), pre)
