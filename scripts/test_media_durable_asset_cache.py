...
class MediaDurableAssetCacheTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        def fake_download(provider: str, url: str, dest: Path) -> Path:
            ...
            payload = (f"raw-{provider}-{url}-{self.network_calls}".encode("utf-8") + b"x" * 8192)
        ...
        def fake_prepare(src: Path, dest: Path, seconds: float, portrait: bool, fps: int = 30) -> Path:
            self.render_calls += 1
            destination = Path(dest)
            destination.parent.mkdir(parents=True, exist_ok=True)
            destination.write_bytes(
                b"prepared-" + Path(src).read_bytes() + f"-{seconds}-{portrait}-{fps}".encode("utf-8")
            )
            return destination
        ...
        self.env = patch.dict(os.environ, {"ISCO_MEDIA_CACHE_DIR": str(self.cache_dir)}, clear=False)
        self.env.start()
        self.trust_hash = patch.object(cache, "_media_trust_contract_sha256", return_value="trust-v1")
        self.render_hash = patch.object(cache, "_render_contract_sha256", return_value="render-v1")
    ...
    def tearDown(self) -> None:
        ...
        if media_trust._quarantine_root is not None and media_trust._quarantine_root != self.original_qroot:
            try:
                import shutil
    ...
    def test_selected_raw_asset_resumes_without_second_network_call_and_rehydrates_trust(self) -> None:
        ...
        first = media_trust.trusted_download("pexels", url, self.run1 / "clips" / "01-raw.mp4")
        ...
        audit = json.loads((self.run2 / cache.AUDIT_FILENAME).read_text(encoding="utf-8"))
    ...
    def test_corrupt_raw_entry_is_evicted_and_reacquired(self) -> None:
        ...
        media_trust.trusted_download("pexels", url, self.run1 / "clips" / "01-raw.mp4")
        ...
        media_trust.trusted_download("pexels", url, self.run2 / "clips" / "01-raw.mp4")
    ...
    def test_prepared_clip_resumes_without_second_render(self) -> None:
        ...
        raw1 = media_trust.trusted_download("pexels", url, self.run1 / "clips" / "01-raw.mp4")
        ...
        self._clear_live_trust()
        raw2 = media_trust.trusted_download("pexels", url, self.run2 / "clips" / "01-raw.mp4")
...
class MediaDurableWorkflowContractTests(unittest.TestCase):
    def test_production_workflow_restores_saves_and_surfaces_media_cache(self) -> None:
        ...
        self.assertIn("ISCO_MEDIA_CACHE_DIR: ${{ runner.temp }}/isco-media-asset-cache", source)
        self.assertIn("Prepare durable selected-media cache for cross-run save", source)
        self.assertIn("Save durable selected-media cache", source)
        self.assertIn("media-asset-v1-${{ runner.os }}-${{ github.run_id }}", source)
