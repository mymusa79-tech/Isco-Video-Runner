...
class ShortContractTests(unittest.TestCase):
    ...
    def test_mistral_gets_one_bounded_hook_only_repair_for_21_word_hook(self) -> None:
        brief = _TEMPLATE_FIXTURES["inner_dialogue"]["brief"]
        plan = _plan(_TEMPLATE_FIXTURES["inner_dialogue"]["queries"])
        original = {
            "title": "شورت",
            "sections": [
                {
                    "id": "s1",
                    "narration": (
                        "حين تفقد الدافع تمامًا قد تظن أن المشكلة فيك بينما أنت في الحقيقة تنتظر شعورًا لن يأتي قبل الحركة الأولى اليوم. "
                ...
                {
                    "id": "s3",
                    "narration": "الخطوة الصغيرة تقلل الاحتكاك وتعيد لك إحساس التقدّم. اختر مهمة واحدة الآن.",
                },
            ],
        }
        repair_response = {
            "title": "عنوان لا يجب استخدامه",
            "sections": [
                {
                    "id": "s1",
                    "narration": "حين يختفي الدافع، قد تكون المشكلة أنك تنتظر الشعور قبل الحركة. نص آخر لا يجب استخدامه.",
                },
        ...
        router = ProviderRouter((ProviderAdapter("mistral", mistral),))
        accepted = router.route(
            stage="script",
            prompt=_script_prompt(brief, plan),
            max_tokens=400,
    ...
    def test_mistral_bounded_hook_repair_refuses_22_word_hook(self) -> None:
        brief = _TEMPLATE_FIXTURES["inner_dialogue"]["brief"]
        plan = _plan(_TEMPLATE_FIXTURES["inner_dialogue"]["queries"])
        overlong = {
            "title": "شورت",
            "sections": [
                {
                    "id": "s1",
                    "narration": "حين تفقد الدافع تمامًا قد تظن أن المشكلة فيك بينما أنت في الحقيقة تنتظر شعورًا لن يأتي قبل الحركة الأولى اليوم فقط.",
                },
                {
                    "id": "s2",
                    "narration": "انتظار الشعور المناسب يطيل التوقف، بينما الحركة الصغيرة تعطيك إشارة جديدة.",
        ...
        router = ProviderRouter((ProviderAdapter("mistral", mistral),))
        with self.assertRaisesRegex(RuntimeError, "script exhausted bounded provider route"):
            router.route(
                stage="script",
                prompt=_script_prompt(brief, plan),
                max_tokens=400,
