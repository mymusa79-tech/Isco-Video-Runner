...
class RuntimeClosureTests(unittest.TestCase):
    ...
    def test_runtime_closure_installs_planning_seam_before_media_audio_and_final_gate(self) -> None:
        calls=[]
        with patch.object(runtime_closure,"install_runtime_planning_contracts",side_effect=lambda:calls.append("planning")) as planning, \
             patch.object(runtime_closure,"install_provider_capacity_v2",side_effect=lambda:calls.append("provider-capacity")) as provider_capacity, \
        ...
        planning.assert_called_once_with(); provider_capacity.assert_called_once_with(); media_trust.assert_called_once_with(); core.assert_called_once_with(); semantic_binding.assert_called_once_with()
        audio.assert_called_once_with(); sfx.assert_called_once_with(); m8.assert_called_once_with(); m9.assert_called_once_with()
        m10.assert_called_once_with(); cta.assert_called_once_with(); music.assert_called_once_with(); bundle.assert_called_once_with(); release.assert_called_once_with(); telemetry.assert_called_once_with()
        modules.assert_called(); semantic_final.assert_called_once()
        self.assertLess(calls.index("planning"), calls.index("media-trust"))
        self.assertLess(calls.index("media-trust"), calls.index("core"))
        ...
        self.assertLess(calls.index("audio"), calls.index("sfx"))
        self.assertEqual(calls[-1], "audio-semantic-final")

    def test_media_trust_and_audio_semantic_order_around_core_and_mastering(self) -> None:
        ...
        self.assertLess(planning_line,trust_line)
