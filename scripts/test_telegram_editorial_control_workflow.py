...
class TelegramEditorialControlWorkflowTests(unittest.TestCase):
    ...
    def test_dedicated_production_workflow_owns_authorization_consumption(self):
        ...
        self.assertIn("steps.poll.outputs.channel_os_consumed == 'true'", persist_block)
    ...
    def test_polling_is_read_only_while_any_production_path_is_active(self):
        ...
        self.assertIn("safe-during-production", self.text)
        self.assertIn('echo "channel_os_consumed=false"', self.text)
        self.assertIn('echo "channel_os_consumed=true"', self.text)
        self.assertIn("Channel OS read-only Mission Control request was served", self.text)
        self.assertIn("non-release non-Channel-OS stateful Telegram command remains read-only", self.text)
