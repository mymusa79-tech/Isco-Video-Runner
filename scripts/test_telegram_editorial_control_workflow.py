...
class TelegramEditorialControlWorkflowTests(unittest.TestCase):
    ...
    def test_control_state_is_encrypted_at_rest(self):
    ...
    def test_schedule_is_dashboard_poll_only(self):
        self.assertIn('cron: "*/5 * * * *"', self.text)
        self.assertIn("python scripts/telegram_control_panel.py poll", self.text)
