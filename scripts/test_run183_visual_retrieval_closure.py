...
class Run183VisualRetrievalClosureTests(unittest.TestCase):
    ...
    def test_opening_ladder_uses_semantic_alternate_not_prop_closeup(self) -> None:
    ...
    def test_enriched_intent_lifts_personal_space_over_whiteboard_marker(self) -> None:
        intent = closure._enriched_build_visual_intent(RUN183_VISUAL)
        self.assertIn("boundary", intent.anchors)
        self.assertIn("relationship", intent.anchors)
    ...
    def test_alternate_pool_excludes_run183_already_reviewed_asset(self) -> None:
        ...
        def fake_selector(*args, **kwargs):
            # Simulate the primary phase having inspected the exact Run183 Pexels asset.
