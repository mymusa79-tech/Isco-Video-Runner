...
Run #242 extends that same ownership boundary to Audio Production V2 during normal Short
finishing. AUDIO_QC_PENDING recovery is deliberately not another Short-finishing scope:
its executor passes the already-captured Gemini capability directly into the Audio resume
contract for both Long and Short recovery.
...
def current_short_finishing_gemini_for_audio() -> str | None:
    """Resolve the authoritative in-memory Gemini lease for normal Short Audio QC.

    ``None`` means no Short-finishing ownership scope is active, allowing Long and
    dedicated resume callers to use their own explicit credential path.  If a Short
    scope is active but its Gemini lease is empty, fail closed instead of escaping to
    environment/file secrets that may already have been consumed.
    """
    capabilities = _ACTIVE.get()
    if capabilities is None:
        return None
