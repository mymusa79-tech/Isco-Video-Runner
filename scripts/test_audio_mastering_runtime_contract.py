...
class AudioMasteringRuntimeContractTests(unittest.TestCase):
    ...
    def test_runtime_closure_and_m7_are_installed_before_produce(self) -> None:
        ...
        m7 = [line for line, name in calls if name == "install_m7_live_binding"]
        produce = [line for line, name in calls if name == "produce"]
        self.assertEqual(len(closure), 1)
        self.assertEqual(len(m7), 1)
        self.assertEqual(len(produce), 1)
        self.assertLess(closure[0], m7[0])
        self.assertLess(m7[0], produce[0])
