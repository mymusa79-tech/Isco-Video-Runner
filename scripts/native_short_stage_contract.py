...
def moment_stage_spec(stage_kind: str, topic: str) -> stage_contract.PlanningStageSpec:
    ...
    return stage_contract.PlanningStageSpec(
        ...
        # compatibility Moment path. We add identity/validation/durability, not a new
        # capacity assumption or a larger retry budget.
...
def _install_stage_contract_extensions() -> None:
    """Extend only the generic Stage Contract schema/semantic dispatch surfaces."""
...
def install_native_short_stage_contract() -> None:
    ...
    # before install_planning_contract_router() is the compatibility bug this closes.
    def contracted_json_text(api_key, prompt, model="gemini-2.5-flash"):
        state = _CALL_STATE.get()
        if state is None:
            # Outside standalone-Moment build lifecycle retain compatibility for
