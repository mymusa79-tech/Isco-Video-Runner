...
class ProviderCapacityWorkflowTests(unittest.TestCase):
    ...
    def test_cache_actions_do_not_gain_repository_write_credentials(self) -> None:
        for action_name in ("Restore", "Persist"):
            match = re.search(
                rf"- name: {action_name} 24h Pixabay API search cache\n(?P<body>.*?)(?=\n\s{{6}}- name:)",
                self.text,
                flags=re.S,
            )
            self.assertIsNotNone(match)
            ...
            self.assertNotIn("persist-credentials", body)
