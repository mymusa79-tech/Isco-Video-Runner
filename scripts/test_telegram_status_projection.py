...
class TelegramStatusProjectionTests(unittest.TestCase):
    def test_waiting_reserved_and_inflight_are_distinct_from_terminal_history(self):
        state = {
            "last_event_at": "2026-08-31T18:20:00+00:00",
            "requests": {},
            "production_queue": [
                {"status": "pending_dispatch", "request_id": "req-wait"},
                {"status": "dispatch_reserved", "request_id": "req-reserved"},
                {"status": "dispatch_consumed", "request_id": "req-running"},
        ...
        projection = build_projection(
            state,
            generated_at=datetime(2026, 8, 31, 18, 21, tzinfo=timezone.utc),
        )["editorial"]

        # Backward-compatible queue depth is only work awaiting V4 ownership.
