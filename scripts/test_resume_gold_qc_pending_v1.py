...
class ResumeGoldQCPendingV1Tests(unittest.TestCase):
    ...
    def test_success_uses_temp_history_and_exports_only_gold_accepted_copy(self) -> None:
        ...
        def fake_gold(*, output_dir, gemini, pexels, pixabay, ledger):
            ...
            temp = Path(__import__("os").environ["ISCO_HISTORY_PATH"])
        ...
        with patch.object(resume, "validate_resume_bundle", return_value=manifest), patch.object(
            resume, "_git_head", side_effect=[RUNNER_SHA, ENGINE_SHA]
            ...
            outcome = resume.execute_gold_resume(
                bundle_dir=bundle,
