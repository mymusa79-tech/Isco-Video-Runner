...
from scripts.orchestration_telegram_ingress_outbox import IngressMode
...
class MasterApprovalContractTests(unittest.TestCase):
    ...
    def test_15_protected_contracts_and_l6_telegram_authority_are_preserved(self):
        expected = {
            "scripts/retry_after_policy.py": "a03252a3df4b511841a6522d93d0400436a84413",
        ...
        # The old PR #375 pinned the pre-L6 telegram_publish_gate blob. L6
        # intentionally replaced that architecture, so the safety contract is now
        # behavioral: webhook-only ingress, no Channel OS Telegram transport owner,
        # and no resurrection of live getUpdates in the publish gate.
        ...
        junction = Path("scripts/channel_os_telegram_junction.py").read_text(encoding="utf-8")
        for forbidden in ("TELEGRAM_BOT_TOKEN", "api.telegram.org", "getUpdates", "requests.post", "gh workflow run"):
            self.assertNotIn(forbidden, junction)
        self.assertIn('"transport_owner": "l6_telegram_outbox"', junction)
        self.assertIn('"publication_authority": "none"', junction)
